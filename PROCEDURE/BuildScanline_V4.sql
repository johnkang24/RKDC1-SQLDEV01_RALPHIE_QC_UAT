...
CREATE PROCEDURE [dbo].[BuildScanline_V4]
	@FileID INT,
	@partial BIT = 0
AS
...
DECLARE @sb_scanline_code__c VARCHAR(MAX), @sb_category__c VARCHAR(MAX), @sb_type__c VARCHAR(MAX), @sb_field_content__c VARCHAR(MAX), @sb_position_length__c INT, @sb_default_text__c VARCHAR(MAX)
DECLARE @sb_scanline_definition VARCHAR(MAX), @sb_bc2d1_definition VARCHAR(MAX), @sb_bc3of9_definition VARCHAR(MAX)
...
DECLARE @audience_type VARCHAR(50), @scanlinetext1 VARCHAR(50), @scanlinetext2 VARCHAR(50)
SELECT @audience_type=SB_RKD_Audience_Code__c, @scanlinetext1=Scanline_Text_1, @scanlinetext2=Scanline_Text_2
...
(
	SB_Scanline_Code__c VARCHAR(MAX),
	SB_Category__c VARCHAR(MAX),
	SB_Type__c VARCHAR(MAX),
...
SET @OPENQUERY = 'SELECT *
	FROM OPENQUERY('+ @LinkedServer + ','''
--SET @OPENQUERY = 'SELECT * FROM OPENQUERY('+ @LinkedServer + ','''
SET @TSQL = 'SELECT SB_Scanline__r.SB_Scanline_Code__c, SB_Scanline__r.SB_Category__c, SB_Scanline__r.SB_Type__c, 
		SB_Field_Content__c
...
IF @testing=1 BEGIN
	...
	FROM #tmpScanlineDefinition A LEFT JOIN [Scanline_Mapping] B ON B.SourceName=A.SB_Field_Content__c
...
-----------------------------------------------------------------------------------
PRINT 'Begin building scanline...'

IF EXISTS(SELECT TOP 1 * FROM #tmpScanlineDefinition A LEFT JOIN [Scanline_Mapping] B ON B.SourceName=A.SB_Field_Content__c
	WHERE A.SB_Category__c='Keyline/Scanline'
		AND SB_Type__c='Built in Ralphie') BEGIN
	UPDATE ORIGINAL SET Scanline_Processed=1 WHERE FileID=@FileID
END

SET @TSQL = ''

DECLARE db_buildscanline CURSOR FOR 
SELECT SB_Scanline_Code__c, SB_Category__c, SB_Type__c, SB_Field_Content__c, SB_Position_Length__c, SB_Default_Text__c, SB_Sequence__c, SourceName, Abbreviation, SourceEnvironment, SourceObject, SourceField
FROM #tmpScanlineDefinition A LEFT JOIN [Scanline_Mapping] B ON B.SourceName=A.SB_Field_Content__c
--WHERE A.SB_Category__c='Keyline/Scanline'
WHERE SB_Type__c<>'Built in Ralphie'
ORDER BY A.SB_Category__c, SB_Sequence__c

OPEN db_buildscanline  
FETCH NEXT FROM db_buildscanline INTO @sb_scanline_code__c, @sb_category__c, @sb_type__c, @sb_field_content__c, @sb_position_length__c, @sb_default_text__c, @sb_sequence__c, @sourcename, @abbreviation, @sourceenvironment, @sourceobject, @sourcefield
...
IF @@FETCH_STATUS = 0 BEGIN
	...
	SET @sb_scanline_definition = ''
	SET @sb_bc2d1_definition = ''
	SET @sb_bc3of9_definition = ''

	--		IF @sb_type__c<>'Built in Ralphie' BEGIN
	WHILE @@FETCH_STATUS = 0  
	BEGIN
		IF @testing=1 BEGIN
			PRINT @sb_category__c + ': ' + @sb_type__c
		END

		IF @sb_type__c='Built in Ralphie' BEGIN
			IF @sb_category__c='Keyline/Scanline' BEGIN
				--UPDATE ORIGINAL SET Scanline_Processed=1 WHERE FileID=@FileID
				SET @TSQL = 'UPDATE ORIGINAL SET Scanline_Processed=1 WHERE FileID='+CAST(@FileID AS VARCHAR(100))
			END
		END ELSE BEGIN
			PRINT 'Generating scanline logic...'
			IF @sb_category__c='Keyline/Scanline' BEGIN
				--PRINT 'Setting @sb_scanline_definition='+@sb_scanline_code__c
				SET @sb_scanline_definition = @sb_scanline_code__c
			END
			IF @sb_category__c='2D Barcode (BC2D1)' BEGIN
				--PRINT 'Setting @sb_bc2d1_definition='+@sb_scanline_code__c
				SET @sb_bc2d1_definition = @sb_scanline_code__c
			END
			IF @sb_category__c='3of9 Barcode' BEGIN
				--PRINT 'Setting @sb_bc3of9_definition='+@sb_scanline_code__c
				SET @sb_bc3of9_definition = @sb_scanline_code__c
			END
			...
			IF @sb_category__c='Keyline/Scanline' BEGIN
				...
				END ELSE BEGIN
					...
					SET @scanline = 'COALESCE(CASE COALESCE('+@tmpVar+ ','''') WHEN '''' THEN NULL ELSE ' + @tmpVar + ' END,' + '''' + TRIM(COALESCE(@sb_default_text__c,'')) + '''' + ')'
				END
				--left zero fill
			...
			IF @sb_category__c='2D Barcode (BC2D1)' BEGIN
				...
				END ELSE BEGIN
					...
					SET @tmpVar = REPLACE(REPLACE(@sourceObject,'SBQQ__Quoteline__c','C'),'ORIGINAL','A') + '.' + @sourcefield
					SET @bc2d = 'COALESCE(CASE COALESCE('+@tmpVar+ ','''') WHEN '''' THEN NULL ELSE ' + @tmpVar + ' END,' + '''' + TRIM(COALESCE(@sb_default_text__c,'')) + '''' + ')'
				END
			...
			IF @sb_category__c='Alt 2D Barcode (BC2D2)' BEGIN
				...
				END ELSE BEGIN
					...
					SET @tmpVar = REPLACE(REPLACE(@sourceObject,'SBQQ__Quoteline__c','C'),'ORIGINAL','A') + '.' + @sourcefield
					SET @bc2d2tmp = 'COALESCE(CASE COALESCE('+@tmpVar+ ','''') WHEN '''' THEN NULL ELSE ' + @tmpVar + ' END,' + '''' + TRIM(COALESCE(@sb_default_text__c,'')) + '''' + ')'
				END
			...
			IF @sb_category__c='3of9 Barcode' BEGIN
				...
				END ELSE BEGIN
					...
					SET @tmpVar = REPLACE(REPLACE(@sourceObject,'SBQQ__Quoteline__c','C'),'ORIGINAL','A') + '.' + @sourcefield
					SET @bc3of9tmp = 'COALESCE(CASE COALESCE('+@tmpVar+ ','''') WHEN '''' THEN NULL ELSE ' + @tmpVar + ' END,' + '''' + TRIM(COALESCE(@sb_default_text__c,'')) + '''' + ')'
				END
				--left zero fill
				IF COALESCE(@sourceenvironment,'None')<>'None' AND COALESCE(@sb_position_length__c,0) > 0 BEGIN
	...
	IF @testing=1 BEGIN
		...
		PRINT '@sb_scanline_definition='+@sb_scanline_definition
	END

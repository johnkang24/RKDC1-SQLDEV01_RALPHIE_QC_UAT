CREATE PROCEDURE [dbo].[LoadSourceFile_V2]
(
...
--SET @FileToLoad = '\\Rkdc1-SQLDEV01\tempdata\RALPHIE\staging\FoodBank\2604\SEVVAMULTITEST\Output\SEVVAMULTITEST_COMS_ORIGINAL.CSV'
--SET @ParentFileID = 0
...
DECLARE @FileNamePath VARCHAR(MAX), @FileType VARCHAR(100), @TotalCount INT, @AdvJobNo VARCHAR(50), @Parent_ID VARCHAR(50),  @Vertical VARCHAR(100), @Client_code VARCHAR(50), @AccountID VARCHAR(50), @Name VARCHAR(MAX)
	...
	*/
	...
	BEGIN TRY
		...
		END ELSE BEGIN
			...
			SET @OPENQUERY = 'SELECT SBQQ__Quote__r_SBQQ__Account__c, SBQQ__Quote__r_SBQQ__Account__r_Name, SBQQ__Quote__r_Advantage_Job_Code__c, SBQQ__Quote__r_SBQQ__Account__r_SB_Vertical__c, SBQQ__Quote__r_SB_Schedule_Block__r_Name, STRING_AGG(Name, '','')
				FROM OPENQUERY('+ @LinkedServer + ','''
			SET @TSQL = 'SELECT SBQQ__Quote__r.SBQQ__Account__c, SBQQ__Quote__r.SBQQ__Account__r.Name, SBQQ__Quote__r.Advantage_Job_Code__c, SBQQ__Quote__r.SBQQ__Account__r.SB_Vertical__c, SBQQ__Quote__r.SB_Schedule_Block__r.Name, Name
				FROM SBQQ__QuoteLine__c WHERE Name IN (''''' + @QuoteLineID + ''''')  '')
				GROUP BY SBQQ__Quote__r_SBQQ__Account__c, SBQQ__Quote__r_SBQQ__Account__r_Name, SBQQ__Quote__r_Advantage_Job_Code__c, SBQQ__Quote__r_SBQQ__Account__r_SB_Vertical__c, SBQQ__Quote__r_SB_Schedule_Block__r_Name
			...
			SELECT TOP 1 @Parent_ID=MIN(parent_id), @Client_code=client_code, @TotalCount=COUNT(*) 
			FROM ORIGINAL 
			WHERE FileID=@FileID 
...
IF @FileType='ORIGINAL' BEGIN
	...
	SELECT AccountID, AccountName, Advantage_Job_Code__c, Vertical, ScheduleBlock_Name, Name
	FROM #tmpVertical
	ORDER BY Advantage_Job_Code__c

	OPEN db_cursor  
	FETCH NEXT FROM db_cursor INTO @AccountID, @AccountName, @AdvJobNo, @Vertical, @ScheduleBlockName, @Name

	WHILE @@FETCH_STATUS = 0  
	BEGIN
		--new job within the same file
		IF @counter > 1 BEGIN
			IF NOT EXISTS(SELECT 1 FROM FileLog WHERE FileNamePath=@FileNamePath AND Advantage_Job_ID=@AdvJobNo AND FileType=@FileType) BEGIN
				...
				SELECT FileNamePath, FileLoaded, LastModifiedDate, FileType, LoadMessage=@LoadMessage, AccountID=@AccountID, AdvJobNo=@AdvJobNo, 
					Vertical=@Vertical, Client_code=@Client_code, AccountName=@AccountName, TotalCount=@TotalCount, ScheduleBlock_Name=@ScheduleBlockName
				FROM FileLog
				WHERE FileID=@primaryFileID

				SELECT @FileID=SCOPE_IDENTITY();
			END ELSE BEGIN
				SELECT @FileID=FileID FROM FileLog WHERE FileNamePath=@FileNamePath AND Advantage_Job_ID=@AdvJobNo AND FileType=@FileType
			END

			--set new FileID for matching data in ORIGINAL
			UPDATE ORIGINAL SET FileID=@FileID
			WHERE parent_id IN (SELECT dbo.udf_ConvertQuotelineToId(value) FROM STRING_SPLIT(@Name,','))

CREATE PROCEDURE [dbo].[Sync_ProcessLog_V1]
(
	@FileID INT
) AS
...
FROM ORIGINAL A JOIN ProcessLog B ON B.Parent_ID=A.parent_id
	AND B.QuoteID=(SELECT QuoteID FROM ProcessLog WHERE FileID=@FileID)
...
WHERE (C.dupedrop IS NULL OR C.dupedrop<>'TRUE')
	OR (C.error_code IS NULL OR C.error_code NOT IN ('-1','111','112','113','114','211','212','213','214','215','216','217','218','219','220','311','312','313','411','412','413','414','415','416','417','418','419','420','421','422','423','491','492','493','494'))

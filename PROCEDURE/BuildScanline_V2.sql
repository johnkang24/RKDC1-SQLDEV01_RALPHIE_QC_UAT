...
CREATE PROCEDURE [dbo].[BuildScanline_V2]
	@FileID INT
AS
...
--	Scanline_Text_2 NVARCHAR(MAX),
--)
SELECT Deliverable_Element__c, MAX([Scanline Text 1]) Scanline_Text_1, MAX([Scanline Text 2]) Scanline_Text_2
INTO #tmpInstructions
	FROM (SELECT Deliverable_Element__c, [Scanline Text 1], [Scanline Text 2]
	FROM 
	(SELECT Deliverable_Element__c, [Name], Instruction_Value__c
		FROM #tmpAllInstructions
		WHERE Instruction_Value__c IS NOT NULL) AS SourceQuery
	PIVOT (
		MAX(Instruction_Value__c)
		FOR [Name] IN ([Scanline Text 1], [Scanline Text 2])
	) AS #PivotTable) A
GROUP BY Deliverable_Element__c
IF @testing=1 BEGIN
	SELECT *
...
IF @testing=1 BEGIN
	SET @TSQL = 'SELECT A.Parent_ID, B.QuoteLine_ID, A.id, A.client_code, A.appeal1, A.rm, C.SB_RKD_Audience_Code__c, A.scanline1, A.scanline2, A.scanline3, A.bc2d1, A.bc2d2, A.bc2d3, A.bc3of9, '
...
SET @TSQL = @TSQL + ' JOIN #tmpInstructions ON B.QuoteLine_ID=#tmpInstructions.Deliverable_Element__c'

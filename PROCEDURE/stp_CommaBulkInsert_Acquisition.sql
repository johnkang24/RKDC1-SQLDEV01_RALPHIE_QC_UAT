...
CREATE PROCEDURE [dbo].[stp_CommaBulkInsert_Acquisition]
	@FileID INT,
	@file NVARCHAR(250), 
	@table NVARCHAR(250),
	@ParentFileID INT=0
...
--SET @file = '\\rkdc1-sqldev01\tempdata\RALPHIE\staging\FoodBank\2601\UAT Testing\P_SHMTN_26-Jan_NHN_596840_132748_original.csv'
--SET @table = 'ORIGINAL_ACQUSITION'
--SET @FileID = 48
--SET @ParentFileID = 0
--SET @testing = 0
...
BEGIN
	...
	CREATE TABLE ##BulkInsert (
		...
		[qc_record] [varchar](10) NULL
	)

	SET @table = REPLACE(@table,'_ACQUSITION','')
	...
	--remove FileID column
	--SET @SQL = 'ALTER TABLE ##BulkInsert ADD qc_record VARCHAR(50) NULL, dpv VARCHAR(50) NULL, m5 VARCHAR(50), m6 VARCHAR(50)'
	----EXECUTE sp_executesql @SQL
	--IF @testing=1 BEGIN
	--	PRINT @SQL
	--END
	...
	--	--remove columns
	--	SET @SQL = 'ALTER TABLE ##BulkInsert DROP COLUMN Original_ID, IsSeed, Sourcecode1, Sourcecode2, Sourcecode3, Sourcecode4'
	--	EXECUTE sp_executesql @SQL
	--END
	--IF @testing=1 BEGIN
	--	SELECT @SQL
	...
	END ELSE BEGIN
		...
		IF @testing=1 BEGIN
			SELECT DISTINCT [parentid] FROM ##BulkInsert

			SELECT *
			FROM ORIGINAL
			WHERE parent_id IN (SELECT DISTINCT parentid FROM ##BulkInsert)
				AND FileID<>@FileID
		--END	ELSE BEGIN
		...
		SET @SQL = @SQL + 'INSERT INTO ' + @table + '(FileID,
				...
				county,STITLE,SFNAME,SMIDDLE,SLNAME,SSUFFIX,IMPORTID,SOLCODE1,SOLCODE2,SOLCODE3,MULTI_FLG,Endorsement,WS,cr_sequence,CR_ID,Income,serial,qc_record,Scanline_Processed)
				SELECT ' + CONVERT(VARCHAR(100),@FileID) + ' FileID,
				Client_Code,ID,salutation,Company,Addressee1,Addressee2,Gender,title,fname,MIDDLE,lname,suffix,EMAIL,PHONE,Address1,address2,City,State,Zip,DPV,GiftDate,GiftAmt,I_GiftDate,
				TotalAmt,MemGifAmt,MemGifDat,RM,LIST,ParentID,ChildID,PLSR_Code,CLSR_Code,MAILTYPE,MAILMNTH,MAILPKG,LTRAMT,UG1,LG,UG2,UG3,UG4,UG5,UG6,M1,MG,M2,M3,M4,M5,M6,
				APPEAL1,APPEAL2,APPEAL3,SCANLINE1,SCANLINE2,SCANLINE3,BC2D1,BC2D2,BC2D3,BC2D4,BC2D5,BC2D6,BC3OF9,MISC1,MISC2,MISC3,MISC4,MISC5,MISC6,MISC7,MISC8,MISC9,
				county,STITLE,SFNAME,SMIDDLE,SLNAME,SSUFFIX,IMPORTID,SOLCODE1,SOLCODE2,SOLCODE3,MULTI_FLG,Endorsement,WS,cr_sequence,CR_ID,Income,ID,0,1
				FROM ##BulkInsert'
	END
